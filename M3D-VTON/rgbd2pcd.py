...
def dilate_rgb(newdepth, pix):
    for _ in range(pix):
        ...
        d3 = newdepth[:, 3:, :]  # right
        d4 = newdepth[:, :-3, :]  # left
        newdepth[:-3, :, :] = np.where(newdepth[:-3, :, :]
                                       > 0, newdepth[:-3, :, :], d1)
        newdepth[3:, :, :] = np.where(
            newdepth[3:, :, :] > 0, newdepth[3:, :, :], d2)
        newdepth[:, :-3, :] = np.where(newdepth[:, :-3, :]
                                       > 0, newdepth[:, :-3, :], d3)
        newdepth[:, 3:, :] = np.where(
            newdepth[:, 3:, :] > 0, newdepth[:, 3:, :], d4)

    return newdepth


def inpaint_back(rgb, parse):
    inpaint_mask = (parse == 13).astype(np.uint8) * 255 + \
        (parse == 10).astype(np.uint8) * 255
...
if __name__ == '__main__':
    ...
    parser.add_argument('--depth_root', type=str,
                        default='results/aligned/DRM/test_pairs/final-depth', help='path to the DRM result')
    parser.add_argument('--frgb_root', type=str, default='results/aligned/TFM/test_pairs/tryon',
                        help='path to the TFM result (front rgb)')
    ...
    parser.add_argument('--parse_root', type=str,
                        default='data/image-parse', help='path to the parsing image')
    parser.add_argument('--point_dst', type=str, default='results/aligned/pcd/test_pairs',
                        help='path to output dir for point cloud')
    ...
    for fd_name in tqdm(depth_list):
        ...
        point_out_fn = os.path.join(
            opt.point_dst, fd_name.replace('_whole_front_depth.npy', '.ply'))
        ...
        depth2point(fd, bd, rgb, rgb_back, parse_shape=parse_shape,
                    label='pred', out_fn=point_out_fn)
