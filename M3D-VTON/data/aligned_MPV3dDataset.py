...
class AlignedMPV3dDataset(BaseDataset):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument('--radius', type=int, default=5,
                            help='radius used when drawing pose keypoints')
    ...
    def __getitem__(self, index):
        ...
        if 'MTM' in self.model:
            c = Image.open(os.path.join(self.dataroot, 'aligned',
                           self.datalist, 'cloth', c_name))
            cm = Image.open(os.path.join(self.dataroot, 'aligned', self.datalist,
                            'cloth-mask', c_name.replace('.jpg', '_mask.jpg')))
        elif 'DRM' in self.model:
            c = Image.open(os.path.join(self.warproot, 'warp-cloth', c_name))
            cm = Image.open(os.path.join(
                self.warproot, 'warp-mask', c_name.replace('.jpg', '_mask.jpg')))
            ...
            raise NotImplementedError(
                'Dataset [%s] is not implemented' % self.model)
        ...
        if self.model == 'MTM':
            ...
            im_parse = Image.open(os.path.join(
                self.dataroot, 'image-parse', parse_name))
            parse_array = np.array(im_parse)
            im_mask = torch.from_numpy(
                (parse_array > 0).astype(np.float32)).unsqueeze(0)
            im_parse = torch.from_numpy(parse_array).float().unsqueeze(0)
        else:
            parse_name = im_name.replace('front.png', 'segmt.png')
            im_parse = Image.open(os.path.join(
                self.warproot, 'segmt', parse_name))
            parse_array = np.array(im_parse)
            im_mask = torch.from_numpy(
                (parse_array > 0).astype(np.float32)).unsqueeze(0)
            im_parse = torch.from_numpy(parse_array).float().unsqueeze(0)
        ...
        if self.model == 'MTM':
            ...
            parse_shape = Image.fromarray(
                (parse_forground*255).astype(np.uint8))
            parse_shape = parse_shape.resize(
                (self.img_width//16, self.img_height//16), Image.BILINEAR)
            parse_shape = parse_shape.resize(
        ...
        if self.model == 'DRM':
            c_sobelx = Image.open(os.path.join(
                self.warproot, 'warp-cloth-sobel', c_name.replace('.jpg', '_sobelx.png'))).convert('L')
            c_sobely = Image.open(os.path.join(
                self.warproot, 'warp-cloth-sobel', c_name.replace('.jpg', '_sobely.png'))).convert('L')
            c_sobelx, c_sobely = self.transform(
                c_sobelx), self.transform(c_sobely)
            ...
            c_sobelx = c_sobelx * pacm - (1 - pacm)
            # [-1,1], fill -1 for other parts
            c_sobely = c_sobely * pacm - (1 - pacm)
        else:
            c_sobelx, c_sobely = '', ''

        # head (exclude neck) & hand & lower body (pants, leg, shoes)
        hand_mask = Image.open(os.path.join(
            self.dataroot, 'palm-mask', im_name.replace('whole_front.png', 'palm_mask.png')))
        ...
        if self.model == 'DRM':
            person_sobelx = Image.open(os.path.join(
                self.dataroot, 'image-sobel', im_name.replace('.png', '_sobelx.png'))).convert('L')
            person_sobely = Image.open(os.path.join(
                self.dataroot, 'image-sobel', im_name.replace('.png', '_sobely.png'))).convert('L')
            person_sobelx, person_sobely = self.transform(
                person_sobelx), self.transform(person_sobely)
            parse_arm = (parse_array == 14).astype(np.float32) + \
                (parse_array == 15).astype(np.float32)
            parse_head_arm_lower = parse_head + parse_arm + parse_lower
            phalm = torch.from_numpy(parse_head_arm_lower).unsqueeze(0)
            imhal_sobelx = person_sobelx * phalm - \
                (1 - phalm)  # [-1,1], fill -1 for other parts
            imhal_sobely = person_sobely * phalm - \
                (1 - phalm)  # [-1,1], fill -1 for other parts
        else:
        ...
        if self.model == 'DRM' or self.model == 'TFM':
            imfd_initial = np.load(os.path.join(
                self.warproot, 'initial-depth', im_name.replace('whole_front.png', 'initial_front_depth.npy')))
            imfd_initial = torch.from_numpy(imfd_initial).unsqueeze(0)
        if self.isTrain:
            imfd = np.load(os.path.join(self.dataroot, 'depth',
                           im_name.replace('.png', '_depth.npy')))
        ...
        imbd_initial = ''
        if self.model == 'DRM':
            imbd_initial = np.load(os.path.join(
                self.warproot, 'initial-depth', im_name.replace('whole_front.png', 'initial_back_depth.npy')))
            imbd_initial = torch.from_numpy(imbd_initial).unsqueeze(0)
        if self.isTrain:
            imbd = np.load(os.path.join(self.dataroot, 'depth',
                           im_name.replace('front.png', 'back_depth.npy')))
        ...
        if self.model == 'MTM':
            pose_path = os.path.join(
                self.dataroot, 'pose', im_name.replace('.png', '_keypoints.json'))
        ...
        if self.model == 'MTM':
            agnostic = torch.cat(
                [im_shape, im_hhl, im_pose_tensor], 0)  # (29,512,320)
        else:
            agnostic = ''

        # grid image
        if self.model == 'MTM':
            im_g = Image.open('M3D-VTON/grid.png')
