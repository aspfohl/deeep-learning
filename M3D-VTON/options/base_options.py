...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--datalist', type=str, default='test_pairs',
                            help='mpv3d data list [train_pairs | test_pairs | test_score]')
        ...
        parser.add_argument('--model', type=str, default='MTM',
                            help='chooses which model to use. [MTM | DRM | TFM]')
        parser.add_argument('--ngf', type=int, default=64,
                            help='# of gen filters in the first conv layer')
        parser.add_argument('--netD', type=str, default='basic',
                            help='specify discriminator architecture [basic | n_layers | pixel | spectral_norm]. The basic model is a 70x70 PatchGAN. n_layers allows you to specify the layers in the discriminator')
        ...
        parser.add_argument('--img_height', type=int,
                            default=512, help='img height in the dataset')
        parser.add_argument('--img_width', type=int,
                            default=320, help='img width in the dataset')
        parser.add_argument('--batch_size', type=int, default=8,
                            help='input batch size (suggested 8 for MTM, 4 for TFM)')
        parser.add_argument('--serial_batches', action='store_true',
                            help='if true, takes images in order to make batches, otherwise takes them randomly')
        parser.add_argument('--num_threads', default=8, type=int,
                            help='# threads for loading data (suggested 8 for MTM, 1 for TFM)')
        parser.add_argument('--no_pin_memory', action='store_true',
                            help='if specified, not pin data to memory')
        ...
        parser.add_argument('--display_winsize', type=int,
                            default=512, help='display window size for HTML')
    ...
    def gather_options(self):
        ...
        self.parser = parser
        # If work in IPython (Jupyter Notebook), you need to pass an empty string: parser.parse_args("")
